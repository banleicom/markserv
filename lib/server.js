...
const createRequestHandler = flags => {
	...
	return (req, res) => {
		...
		const decodedUrl = getPathFromUrl(decodeURIComponent(req.originalUrl));
		// 如果 decodedUrl 为空或仅为根路径，则将其设置为 '/index.md'
		const effectiveDecodedUrl = (decodedUrl === '' || decodedUrl === '/') ? '/index.md' : decodedUrl;
		// 生成完整的文件路径
		const filePath = path.normalize(unescape(dir) + unescape(effectiveDecodedUrl));
